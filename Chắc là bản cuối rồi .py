...
class NewtonSolver:
    ...
    def solve(self, equation_str, user_tolerance):
        ...
        search_interval = domain.intersect(sp.Interval(-100, 100))
...
if __name__ == '__main__':
    user_equation = input("Nhập phương trình f(x) = 0: ")
    try:
        user_tolerance_str = input("Nhập sai số mong muốn (tối thiểu 10^-7): ")
        user_tolerance = float(user_tolerance_str)
    except ValueError:
        print("Sai số không hợp lệ. Sử dụng giá trị mặc định là 1e-6.")
        user_tolerance = 1e-6
    
    # 1. Tạo đối tượng solver
    solver = NewtonSolver()
    
    # 2. Gọi hàm .solve() để nhận về DICTIONARY KẾT QUẢ
    ket_qua_dict = solver.solve(user_equation, user_tolerance)
    
    # 3. (Tùy chọn) Dùng hàm helper để định dạng dictionary thành chuỗi và in ra
    ket_qua_str = format_output_string(ket_qua_dict)
    print(ket_qua_str)